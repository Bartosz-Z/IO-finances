...
class Evaluator:
    ...
    def plot_result(self, genotype, ax):
        ...
        return Solution(roi, mdd, genotype)
    ...
    def evaluate(self, genotypes, results):
        ...
        for i in range(genotypes.shape[0]):
            ...
            for j in range(len(self._unique_inds)):
                if abs(results[self._unique_inds[j]][0] - roi) < 0.00001 and abs(results[self._unique_inds[j]][1] - mdd) < 0.00001:
                    found = True
                    break
            if not found:
        ...
        for i in range(len(self._unique_inds) // 4):
            ...
            self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[i * 4]], ax=axs[0, 0]))
            self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[i * 4 + 1]], ax=axs[0, 1]))
            self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[i * 4 + 2]], ax=axs[1, 0]))
            self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[i * 4 + 3]], ax=axs[1, 1]))
        ...
        if len(self._unique_inds) % 4 > 0:
            ...
            if len(self._unique_inds) % 4 >= 1:
                self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[idx]], ax=axs[0, 0]))
            if len(self._unique_inds) % 4 >= 2:
                self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[idx + 1]], ax=axs[0, 1]))
            if len(self._unique_inds) % 4 >= 3:
                self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[idx + 2]], ax=axs[1, 0]))
            if len(self._unique_inds) % 4 >= 4:
                self._solutions.append(self.plot_result(genotype=genotypes[self._unique_inds[idx + 3]], ax=axs[1, 1]))
            fig.legend(['Total money for model', 'Exchange rate'], loc='upper center', ncol=2)
